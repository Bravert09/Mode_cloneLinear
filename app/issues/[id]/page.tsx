    ...
    <div className='max-w-4xl mx-auto p-4 md:p-8'>
            ...
            <Link href={`/issues/${id}/edit`}>
              <Button variant='outline' size='sm'>
                <span className='flex items-center'>
                    <Edit2Icon size={16} className='mr-1'/> Edit</span>
              </Button>
            </Link>
      ...
      <div className='bg-white dark:bg-dark-elevated border border-gray-200 dark:border-dark-border-default rounded-lg shadow-sm p-6 mb-8'>
        <div className='flex flex-wrap gap-3 mb-6'>
          ...
          <Badge>{getPriorityLabel(priority)}</Badge>
          <div className='text-sm text-gray-500'>Created {formatRelativeTime(new Date(createdAt))}</div>
          <div className='text-sm text-gray-500'>Updated {formatRelativeTime(new Date(updatedAt))}</div>
        </div>
        {description ? (
